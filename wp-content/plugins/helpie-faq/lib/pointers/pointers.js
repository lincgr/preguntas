...
(function($, MAP) {
    ...
    MAP.hasNext = function(data) {
        // check if a given pointer has valid next property
        return (
    ...
    MAP.isVisible = function(data) {
        // check if anchor for given pointer is visible
    ...
    MAP.setNext = function($widget, data) {
        if (typeof $widget === "object") {
            ...
            has_next = false;
            if (MAP.hasNext(data)) {
                has_next_data = MAP.getPointerData(
                    MAP.js_pointers[data.next].data
                );
