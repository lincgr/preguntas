 ...
 *
 */
class HFPSFramework_Option_Sorter extends HFPSFramework_Options {
  ...
  public function output(){
    ...
    $enabled_title  = ( isset( $this->field['enabled_title'] ) ) ? $this->field['enabled_title'] : esc_html__( 'Enabled Modules', 'hfps-framework' );
    $disabled_title = ( isset( $this->field['disabled_title'] ) ) ? $this->field['disabled_title'] : esc_html__( 'Disabled Modules', 'hfps-framework' );

    echo '<div class="hfps-modules">';
    echo '<h3>'. $enabled_title .'</h3>';
    echo '<ul class="hfps-enabled">';
    if( ! empty( $enabled ) ) {
