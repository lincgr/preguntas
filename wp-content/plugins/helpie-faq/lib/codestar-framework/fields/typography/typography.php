 ...
 *
 */
class HFPSFramework_Option_typography extends HFPSFramework_Options {
  ...
  public function output() {
    ...
    $default_variants = apply_filters( 'hfps_websafe_fonts_variants', array(
      'regular',
      'italic',
      '700',
      '700italic',
    ...
    $websafe_fonts = apply_filters( 'hfps_websafe_fonts', array(
      'Arial',
      'Arial Black',
      'Comic Sans MS',
      'Impact',
    ...
    $google_json   = hfps_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<label class="hfps-typography-family">';
      echo '<select name="'. $this->element_name( '[family]' ) .'" class="'. $is_chosen . $chosen_rtl .'hfps-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action( 'hfps_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_html__( 'Web Safe Fonts', 'hfps-framework' ) .'">';
      ...
      echo '<optgroup label="'. esc_html__( 'Google Fonts', 'hfps-framework' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        echo '<label class="hfps-typography-variant">';
        echo '<select name="'. $this->element_name( '[variant]' ) .'" class="'. $is_chosen . $chosen_rtl .'hfps-typo-variant" data-atts="variant">';
      ...
      echo '<input type="text" name="'. $this->element_name( '[font]' ) .'" class="hfps-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';

    } else {

      echo esc_html__( 'Error! Can not load json file.', 'hfps-framework' );
