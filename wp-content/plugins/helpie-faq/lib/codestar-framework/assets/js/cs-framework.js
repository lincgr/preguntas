...
;(function ( $, window, document, undefined ) {
  ...
  // caching variables
  var hfps_is_rtl  = $hfps_body.hasClass('rtl');
  ...
  $.fn.CSFRAMEWORK_TAB_NAVIGATION = function() {
    return this.each(function() {
      ...
      var $this   = $(this),
          $nav    = $this.find('.hfps-nav'),
          $reset  = $this.find('.hfps-reset'),
          $expand = $this.find('.hfps-expand-all');
      ...
      $nav.find('ul:first a').on('click', function (e) {
        ...
        if( $next.is('ul') ) {
          ...
          $el.closest('li').toggleClass('hfps-tab-active');

        } else {

          $('#hfps-tab-'+$target).show().siblings().hide();
          $nav.find('a').removeClass('hfps-section-active');
          $el.addClass('hfps-section-active');
          $reset.val($target);
      ...
      $expand.on('click', function (e) {
        e.preventDefault();
        $this.find('.hfps-body').toggleClass('hfps-show-all');
  ...
  $.fn.CSFRAMEWORK_SORTER = function() {
    return this.each(function() {
      ...
      var $this     = $(this),
          $enabled  = $this.find('.hfps-enabled'),
          $disabled = $this.find('.hfps-disabled');
      ...
      $enabled.sortable({
        ...
        update: function( event, ui ){
          ...
          if( ui.item.parent().hasClass('hfps-enabled') ) {
  ...
  $.fn.CSFRAMEWORK_IMAGE_UPLOADER = function() {
    return this.each(function() {
      ...
      var $this    = $(this),
          $add     = $this.find('.hfps-add'),
          $preview = $this.find('.hfps-image-preview'),
          $remove  = $this.find('.hfps-remove'),
          $input   = $this.find('input'),
          $img     = $this.find('img'),
          wp_media_frame;
  ...
  $.fn.CSFRAMEWORK_IMAGE_GALLERY = function() {
    return this.each(function() {
      ...
      $this.on('click', '.hfps-add, .hfps-edit', function( e ) {

        var $el   = $(this),
            ids   = $input.val(),
            what  = ( $el.hasClass('hfps-edit') ) ? 'edit' : 'add',
  ...
  $.fn.CSFRAMEWORK_TYPOGRAPHY = function() {
    return this.each( function() {
      ...
      var typography      = $(this),
          family_select   = typography.find('.hfps-typo-family'),
          variants_select = typography.find('.hfps-typo-variant'),
          typography_type = typography.find('.hfps-typo-font');
  ...
  $.fn.CSFRAMEWORK_GROUP = function() {
    return this.each(function() {
      ...
      var _this           = $(this),
          field_groups    = _this.find('.hfps-groups'),
          accordion_group = _this.find('.hfps-accordion'),
          clone_group     = _this.find('.hfps-group:first').clone();

      if ( accordion_group.length ) {
        accordion_group.accordion({
          header: '.hfps-group-title',
          collapsible : true,
          active: false,
          animate: 250,
          heightStyle: 'content',
      ...
      field_groups.sortable({
        axis: 'y',
        handle: '.hfps-group-title',
        helper: 'original',
        cursor: 'move',
        placeholder: 'widget-placeholder',
        start: function( event, ui ) {
          var inside = ui.item.children('.hfps-group-content');
        ...
        stop: function( event, ui ) {
          ui.item.children( '.hfps-group-title' ).triggerHandler( 'focusout' );
          accordion_group.accordion({ active:false });
        }
      });

      var i = 0;
      $('.hfps-add-group', _this).on('click', function( e ) {
      ...
      field_groups.on('click', '.hfps-remove-group', function(e) {
        e.preventDefault();
        $(this).closest('.hfps-group').remove();
      });
    ...
    return this.each( function() {
      ...
      $this.on('click', function ( e ) {
        ...
        if( $ajax.length ) {
          ...
          var serializedOptions = $('#hfpsframework_form').serialize();
    ...
    return this.each( function() {
      ...
      if( $parent.attr('id') === 'addtag' ) {
        ...
        var $submit  = $parent.find('#submit'),
            ...
            $wrap    = $parent.find('.hfps-framework'),
            $clone   = $wrap.find('.hfps-element').clone(),
            $list    = $('#the-list'),
            flooding = false;
  ...
  $.CSFRAMEWORK.ICONS_MANAGER = function() {
    ...
    base.init = function () {
      ...
      $hfps_body.on('click', '.hfps-icon-add', function( e ) {

        e.preventDefault();

        var $this   = $(this),
            $dialog = $('#hfps-icon-dialog'),
            $load   = $dialog.find('.hfps-dialog-load'),
            $select = $dialog.find('.hfps-dialog-select'),
            $insert = $dialog.find('.hfps-dialog-insert'),
            $search = $dialog.find('.hfps-icon-search');

        // set parent
        $parent = $this.closest('.hfps-icon-select');
        ...
        $dialog.dialog({
          ...
          open: function() {
            ...
            // fix scrolling
            $hfps_body.addClass('hfps-icon-scrolling');
          ...
          close: function() {
            $hfps_body.removeClass('hfps-icon-scrolling');
          }
        });
          ...
          $.ajax({
            ...
            success: function( content ) {
              ...
              $load.on('click', 'a', function( e ) {
                ...
                var icon = $(this).data('hfps-icon');

                $parent.find('i').removeAttr('class').addClass(icon);
                $parent.find('input').val(icon).trigger('change');
                $parent.find('.hfps-icon-preview').removeClass('hidden');
                $parent.find('.hfps-icon-remove').removeClass('hidden');
                $dialog.dialog('close');
              ...
              $search.keyup( function(){
                ...
                $icons.each(function() {
                  ...
                  if ( $ico.data('hfps-icon').search( new RegExp( value, 'i' ) ) < 0 ) {
                    $ico.hide();
                  } else {
                    $ico.show();
                  }
              ...
              $load.find('.hfps-icon-tooltip').hfpstooltip({html:true, placement:'top', container:'body'});
      ...
      $hfps_body.on('click', '.hfps-icon-remove', function( e ) {
        ...
        $parent.find('.hfps-icon-preview').addClass('hidden');
  ...
  $.CSFRAMEWORK.SHORTCODE_MANAGER = function() {
    ...
    base.init = function () {
      ...
      var $dialog          = $('#hfps-shortcode-dialog'),
          $insert          = $dialog.find('.hfps-dialog-insert'),
          $shortcodeload   = $dialog.find('.hfps-dialog-load'),
          $selector        = $dialog.find('.hfps-dialog-select'),
      ...
      $hfps_body.on('click', '.hfps-shortcode', function( e ) {
        ...
        shortcode_target  = $shortcode_button.hasClass('hfps-shortcode-textarea');
        ...
        $dialog.dialog({
          ...
          open: function() {
            ...
            // fix scrolling
            $hfps_body.addClass('hfps-shortcode-scrolling');
          ...
          close: function() {
            shortcode_target = false;
            $hfps_body.removeClass('hfps-shortcode-scrolling');
          }
        });
        ...
        if( shortcode_name.length ){
          ...
          $.ajax({
            ...
            success: function( content ) {
              ...
              shortcode_clone = $('.hfps-shortcode-clone', $dialog).clone();
      ...
      $insert.on('click', function ( e ) {
        ...
        switch ( shortcode_view ){
          ...
          case 'contents':
            ...
            $('[' + ruleAttr + ']', '.hfps-dialog-load').each( function() {
          ...
          case 'clone':
            ...
            $('[' + ruleAttr + ']', '.hfps-dialog-load .hfps-element:not(.hidden)').each( function() {
            ...
            $('[' + cloneID + ']', '.hfps-dialog-load').each( function() {
                ...
                $('[' + cloneAttr + ']', _this_clone.find('.hfps-element').not('.hidden') ).each( function() {
      ...
      $dialog.on('click', '#shortcode-clone-button', function( e ) {
        ...
        $('.hfps-shortcode-clone:last').after( cloned_el );

        // add - remove effects
        cloned_el.slideDown(100);

        cloned_el.find('.hfps-remove-clone').show().on('click', function( e ) {
    ...
    base.validate_atts = function( _atts, _this ) {
      ...
      if( _this.data('check') !== undefined ) {
        el_value = _this.closest('.hfps-element').find('input:checked').map( function() {
         return $(this).val();
        }).get();
      }
      ...
      if( $this.data('rgba') !== false ) {
        ...
        $this.wpColorPicker({
          ...
          change: function( event, ui ) {
            ...
            $this.closest('.wp-picker-container').find('.hfps-alpha-slider-offset').css('background-color', ui_color_value);
          ...
          create: function() {
            ...
            var a8cIris       = $this.data('a8cIris'),
                ...
                $alpha_wrap   = $('<div class="hfps-alpha-wrap">' +
                                  '<div class="hfps-alpha-slider"></div>' +
                                  '<div class="hfps-alpha-slider-offset"></div>' +
                                  '<div class="hfps-alpha-text"></div>' +
                                  '</div>').appendTo( $container.find('.wp-picker-holder') ),

                $alpha_slider = $alpha_wrap.find('.hfps-alpha-slider'),
                $alpha_text   = $alpha_wrap.find('.hfps-alpha-text'),
                $alpha_offset = $alpha_wrap.find('.hfps-alpha-slider-offset');
            ...
            $alpha_slider.slider({
              ...
              create: function() {
                ...
                $hfps_body.on( 'click.wpcolorpicker', function() {
                  $alpha_wrap.hide();
                });
  ...
  $.fn.CSFRAMEWORK_TOOLTIP = function() {
    return this.each(function() {
      var placement = ( hfps_is_rtl ) ? 'right' : 'left';
      $(this).hfpstooltip({html:true, placement:placement, container:'body'});
    });
  };
  ...
  $.fn.CSFRAMEWORK_RELOAD_PLUGINS = function() {
    return this.each(function() {
      ...
      $('.hfps-field-image-select', this).CSFRAMEWORK_IMAGE_SELECTOR();
      $('.hfps-field-image', this).CSFRAMEWORK_IMAGE_UPLOADER();
      $('.hfps-field-gallery', this).CSFRAMEWORK_IMAGE_GALLERY();
      $('.hfps-field-sorter', this).CSFRAMEWORK_SORTER();
      $('.hfps-field-upload', this).CSFRAMEWORK_UPLOADER();
      $('.hfps-field-typography', this).CSFRAMEWORK_TYPOGRAPHY();
      $('.hfps-field-color-picker', this).CSFRAMEWORK_COLORPICKER();
      $('.hfps-help', this).CSFRAMEWORK_TOOLTIP();
    });
  };
  ...
  $(document).ready( function() {
    $('.hfps-framework').CSFRAMEWORK_TAB_NAVIGATION();
    $('.hfps-reset-confirm, .hfps-import-backup').CSFRAMEWORK_CONFIRM();
    $('.hfps-content, .wp-customizer, .widget-content, .hfps-taxonomy').CSFRAMEWORK_DEPENDENCY();
    $('.hfps-field-group').CSFRAMEWORK_GROUP();
    $('.hfps-save').CSFRAMEWORK_SAVE();
    $('.hfps-taxonomy').CSFRAMEWORK_TAXONOMY();
    $('.hfps-framework, #widgets-right').CSFRAMEWORK_RELOAD_PLUGINS();
