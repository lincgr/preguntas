...
* Extract download zip on `themename/hfps-framework` folder under your theme directory
* Add framework include code on your theme `themename/functions.php` file

```php
require_once dirname( __FILE__ ) .'/hfps-framework/hfps-framework.php';
// -(or)-
require_once get_template_directory() .'/hfps-framework/hfps-framework.php';
```

* Yay! Right now you are ready to configure framework, metaboxes, taxonomies, wp customize, shortcoder
* Take a look for config files from `themename/hfps-framework/config` folder
* Read for more from [documentation](http://codestarframework.com/documentation/)

##### B) Usage as Plugin
* Download zip file from github repository
* **Way1** Extract download zip on `wp-content/plugins/hfps-framework` folder under your plugin directory
...
* Yay! Right now you are ready to configure framework, metaboxes, taxonomies, wp customize, shortcoder
* Take a look for config files from `wp-content/plugins/hfps-framework/config` folder also you can manage config files from theme directory. see overriding files method.
* Read for more from [documentation](http://codestarframework.com/documentation/)
...
```
or take a look for change define base code from `/hfps-framework/hfps-framework.php` directly.
...
## Overriding Files
You can override an existing file without change `themename/hfps-framework` folder. just create one `themename/hfps-framework-override` folder on your theme directory. for eg:

```php
themename/hfps-framework-override/config/framework.config.php
themename/hfps-framework-override/functions/constants.php
themename/hfps-framework-override/fields/text/text.php
```
