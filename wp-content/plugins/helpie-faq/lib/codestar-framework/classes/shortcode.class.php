 ...
 *
 */
class HFPSFramework_Shortcode_Manager extends HFPSFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'hfps_shortcode_options', $options );
    $this->exclude_post_types = apply_filters( 'hfps_shortcode_exclude', $this->exclude_post_types );
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_hfps-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && HFPS_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button hfps-shortcode" style="background-color: #61ce70; color: white; border: 0px;" data-editor-id="'. $editor_id .'">'. esc_html__( 'Add Helpie Shortcode', 'hfps-framework' ) .'</a>';
    }
  ...
  ?>
    <div id="hfps-shortcode-dialog" class="hfps-dialog" title="<?php esc_html_e( 'Add Helpie Shortcode', 'hfps-framework' ); ?>">
      <div class="hfps-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>hfps-dialog-select" data-placeholder="<?php esc_html_e( 'Select a shortcode', 'hfps-framework' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="hfps-dialog-load"></div>
      <div class="hfps-insert-button hidden">
        <a href="#" class="button button-primary hfps-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'hfps-framework' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    $request = hfps_get_var( 'shortcode' );
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        echo hfps_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="hfps-shortcode-clone" data-clone-id="'. $clone_id .'">';
      echo '<a href="#" class="hfps-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      echo '<div class="hfps-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
