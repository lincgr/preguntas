 ...
 *
 */
class HFPSFramework_Taxonomy extends HFPSFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'hfps_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && HFPS_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = hfps_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $hfps_errors = get_transient( 'hfps-taxonomy-transient' );

    wp_nonce_field( 'hfps-taxonomy', 'hfps-taxonomy-nonce' );

    echo '<div class="hfps-framework hfps-taxonomy hfps-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo hfps_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( hfps_get_var( 'hfps-taxonomy-nonce' ), 'hfps-taxonomy' ) ) {

      $errors = array();
      $taxonomy = hfps_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'hfps_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'hfps_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'hfps_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'hfps_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'hfps_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'hfps-taxonomy-transient', $errors, 10 );
