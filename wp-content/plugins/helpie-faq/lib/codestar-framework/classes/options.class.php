 ...
 *
 */
abstract class HFPSFramework_Options extends HFPSFramework_Abstract {
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="hfps-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'HFPS_OPTIONS_DEBUG' ) && HFPS_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'hfps-framework' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". esc_html__( 'USAGE', 'hfps-framework' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "hfps_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'hfps-framework' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'HFPS_OPTIONS_DEBUG_LIGHT' ) && HFPS_OPTIONS_DEBUG_LIGHT ) ) {
      ...
      $out .= ( isset( $this->field['id'] ) ) ? "hfps_get_option( '". $this->field['id'] ."' );" : '';
      $out .= "\n";
      $out .= "<strong>". esc_html__( 'ID', 'hfps-framework' ) .":</strong>";
      $out .= "\n";
  ...
  public function element_get_error() {
    ...
    if( ! empty( $hfps_errors ) ) {
      foreach ( $hfps_errors as $key => $value ) {
        if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
          $out .= '<p class="hfps-text-warning">'. $value['message'] .'</p>';
        }
      }
    }
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="hfps-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      foreach ( $this->multilang['languages'] as $key => $val ) {
        ...
        if( $key != $this->multilang['current'] ) {
          ...
          $class = 'HFPSFramework_Option_' . $this->field['type'];
      ...
      $out .= '</fieldset>';
      $out .= '<p class="hfps-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'hfps-framework' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? hfps_language_defaults() : false;
  }

}

// load all of fields
hfps_load_option_fields();
